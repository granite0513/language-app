...
export default function LanguageLearningApp() {
  ...
  const normalizeText = (text) => {
    const slangMap = {
      'ta': 'esta', 'tá': 'esta', 'cê': 'voce', 'vc': 'voce',
      'pra': 'para', 'pro': 'para', 'tô': 'estou',
    };
  ...
  const saveEditedPhrase = () => {
    setVocabulary(vocabulary.map(v =>
      v.id === editingId ? { ...v, category: editCategory, difficulty: editDifficulty } : v
    ));
    setEditingId(null);
    setFeedback('Updated');
    setTimeout(() => setFeedback(''), 2000);
  ...
  const getQuizVocabulary = () => {
    ...
    if (quizCategory !== 'All') filtered = filtered.filter(v => v.category === quizCategory);
    if (quizDifficulty !== 'All') filtered = filtered.filter(v => v.difficulty === quizDifficulty);
    return filtered;
  };

  const getCategories = () => ['All', ...new Set(vocabulary.map(v => v.category))];
  const getDifficulties = () => ['All', 'easy', 'medium', 'hard'];

  const getProgress = () => ({
    total: vocabulary.length,
    mastered: vocabulary.filter(v => v.correctCount >= 3).length,
    accuracy: quizStats.attempted > 0 ? Math.round((quizStats.correct / quizStats.attempted) * 100) : 0
  });
  ...
  const getFilteredVocabulary = () => {
    ...
    if (selectedDifficulty !== 'All') filtered = filtered.filter(v => v.difficulty === selectedDifficulty);
    return filtered;
  };
    ...
    return (
      <div style={{ backgroundColor: '#f8f9fa', minHeight: '100vh', padding: '20px' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          <button onClick={() => setMode('home')} style={{ padding: '8px 16px', border: 'none', borderRadius: '6px', backgroundColor: '#f0f0f0', cursor: 'pointer', marginBottom: '20px' }}>Back to Home</button>
          <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <button onClick={() => setQuizDirection('en-to-pt')} style={{ padding: '24px', border: 'none', borderRadius: '12px', backgroundColor: '#2196F3', color: 'white', fontSize: '16px', fontWeight: '600', cursor: 'pointer' }}>English to Portuguese</button>
              <button onClick={() => setQuizDirection('pt-to-en')} style={{ padding: '24px', border: 'none', borderRadius: '12px', backgroundColor: '#FF9800', color: 'white', fontSize: '16px', fontWeight: '600', cursor: 'pointer' }}>Portuguese to English</button>
            </div>
          </div>
        </div>
      </div>
  ...
  if (mode === 'quiz' && quizDirection && !quizComplete) {
    ...
    const checkAnswerNow = () => {
      if (checkAnswerMatch(userAnswer, correctAnswer)) {
        setFeedback('CORRECT');
        setVocabulary(vocabulary.map(v => 
          v.id === current.id ? { ...v, timesReviewed: v.timesReviewed + 1, correctCount: v.correctCount + 1 } : v
        ));
        ...
        setVocabulary(vocabulary.map(v => 
          v.id === current.id ? { ...v, timesReviewed: v.timesReviewed + 1 } : v
        ));
    ...
    return (
      <div style={{ backgroundColor: '#f8f9fa', minHeight: '100vh', padding: '20px' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          <button onClick={() => { setMode('home'); setQuizDirection(null); }} style={{ padding: '8px 16px', border: 'none', borderRadius: '6px', backgroundColor: '#f0f0f0', cursor: 'pointer', marginBottom: '20px' }}>Back to Home</button>
          <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', marginBottom: '20px' }}>
              <button onClick={handleSpeak} style={{ flex: 1, padding: '12px', border: 'none', borderRadius: '8px', backgroundColor: '#FF9800', color: 'white', fontSize: '16px', fontWeight: '600', cursor: 'pointer' }}>Speak</button>
              <button onClick={handleListen} style={{ flex: 1, padding: '12px', border: 'none', borderRadius: '8px', backgroundColor: '#4CAF50', color: 'white', fontSize: '16px', fontWeight: '600', cursor: 'pointer' }}>Record</button>
            </div>
            ...
            <button onClick={checkAnswerNow} disabled={!userAnswer.trim()} style={{ width: '100%', padding: '12px', border: 'none', borderRadius: '8px', backgroundColor: !userAnswer.trim() ? '#ccc' : '#2196F3', color: 'white', fontSize: '16px', fontWeight: '600', cursor: 'pointer' }}>Check Answer</button>
          </div>
        </div>
      </div>
    );
      ...
      <div style={{ backgroundColor: '#f8f9fa', minHeight: '100vh', padding: '20px' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', textAlign: 'center' }}>
            ...
            <button onClick={() => setMode('home')} style={{ padding: '12px 24px', border: 'none', borderRadius: '8px', backgroundColor: '#2196F3', color: 'white', fontSize: '16px', fontWeight: '600', cursor: 'pointer' }}>Back to Home</button>
          </div>
        </div>
      </div>
    );
      ...
      <div style={{ backgroundColor: '#f8f9fa', minHeight: '100vh', padding: '20px' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '30px' }}>
            <button onClick={() => setMode('input')} style={{ padding: '24px', border: 'none', borderRadius: '12px', backgroundColor: '#4CAF50', color: 'white', fontSize: '16px', fontWeight: '600', cursor: 'pointer' }}>Add Vocabulary</button>
            <button onClick={startQuiz} style={{ padding: '24px', border: 'none', borderRadius: '12px', backgroundColor: '#2196F3', color: 'white', fontSize: '16px', fontWeight: '600', cursor: 'pointer' }}>Start Quiz</button>
          </div>
          ...
          <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
              <button onClick={() => { 
                ...
                link.click();
              }} style={{ padding: '8px 16px', border: 'none', borderRadius: '6px', backgroundColor: '#4CAF50', color: 'white', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>Download</button>
              ...
              <button onClick={() => document.getElementById('fileInput').click()} style={{ padding: '8px 16px', border: 'none', borderRadius: '6px', backgroundColor: '#2196F3', color: 'white', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>Upload</button>
            </div>
                    ...
                    return (
                      <div key={item.id} style={{ padding: '16px', backgroundColor: '#e8f5e9', borderRadius: '8px', border: '2px solid #4CAF50' }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px' }}>
                          <button onClick={saveEditedPhrase} style={{ flex: 1, padding: '8px', border: 'none', borderRadius: '6px', backgroundColor: '#4CAF50', color: 'white', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>Save</button>
                          <button onClick={() => setEditingId(null)} style={{ flex: 1, padding: '8px', border: 'none', borderRadius: '6px', backgroundColor: '#f0f0f0', color: '#333', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>Cancel</button>
                        </div>
                      </div>
                    );
                  }
                    ...
                    <div key={item.id} style={{ padding: '12px', backgroundColor: '#f5f5f5', borderRadius: '8px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '6px' }}>
                          <button onClick={() => startEditingPhrase(item)} style={{ padding: '6px 10px', border: 'none', borderRadius: '6px', backgroundColor: '#e3f2fd', color: '#1976d2', cursor: 'pointer', fontSize: '12px', fontWeight: '600' }}>Edit</button>
                          <button onClick={() => setVocabulary(vocabulary.filter(v => v.id !== item.id))} style={{ padding: '6px 10px', border: 'none', borderRadius: '6px', backgroundColor: '#ffebee', color: '#c62828', cursor: 'pointer', fontSize: '12px', fontWeight: '600' }}>Delete</button>
                        </div>
                      </div>
      ...
      <div style={{ backgroundColor: '#f8f9fa', minHeight: '100vh', padding: '20px' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          ...
          <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
            ...
            {feedback && <div style={{ padding: '12px', backgroundColor: '#d4edda', borderRadius: '6px', marginBottom: '16px', fontSize: '14px', color: '#155724' }}>{feedback}</div>}
            <button onClick={addVocabulary} style={{ width: '100%', padding: '12px', border: 'none', borderRadius: '8px', backgroundColor: '#4CAF50', color: 'white', fontSize: '16px', fontWeight: '600', cursor: 'pointer' }}>Add Vocabulary</button>
          </div>
        </div>
      </div>
    );
